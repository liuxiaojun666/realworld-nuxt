        ...
        <div class="col-md-6 offset-md-3 col-xs-12">
          ...
          <ul class="error-messages">
            <template v-for="(messages, field) in errors">
              <li v-for="msg in messages" :key="msg">{{ field + ' ' + msg }}</li>
            </template>
          </ul>
          ...
          <form @submit.prevent="onSubmit">
            ...
            <fieldset class="form-group">
              <input v-model="user.password" class="form-control form-control-lg" type="password" required minlength="8" placeholder="Password">
            </fieldset>
...
export default {
  ...
  methods: {
    async onSubmit () {
      try {
        const { data } = await (this.isLogin ? login : register)({ user: this.user })
